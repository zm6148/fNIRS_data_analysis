function [ b, r, p ] = RCS_breath_onset_time( dc, s_b, RC, channels, Hb_type, onset_time)
...
dc_HbO_RC_sum = 0;
for ii = 1:length(RC)
    dc_HbO_RC_sum = dc_HbO_RC_sum + dc(:,number,RC(ii)); %/std(dc_task(:,1,RC(ii)));
end
...
for ii = 1:length(channels)
    dc_HbO_breath_channels_sum = dc_HbO_breath_channels_sum + dc(:,number,channels(ii)); %/std(dc_task(:,1,channels(ii)));
end
