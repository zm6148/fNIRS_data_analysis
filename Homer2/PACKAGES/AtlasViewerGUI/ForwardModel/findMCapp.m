...
% Check Desktop for MC app 
[mc_exename, mc_appname, ext] = searchDirForMCApp(desktopDir);
if ~isempty(mc_exename)
    fwmodel.mc_rootpath = desktopDir;
    fwmodel.mc_exepath = desktopDir;
    fwmodel.mc_exename = mc_exename;
    fwmodel.mc_appname = mc_appname;
    fwmodel.mc_exename_ext = ext;
    return;
...
for ii=1:length(mc_appnamelist)
    mc_exepath = sprintf('%s/%s/bin/%s', desktopDir, mc_appnamelist{ii}, platform);
    [mc_exename, mc_appname, ext] = searchDirForMCApp(mc_exepath);
    if ~isempty(mc_exename)
