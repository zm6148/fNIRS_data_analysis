function fwmodel = genFluenceProfile_scalp(fwmodel, probe, iFirst, iLast, headvol, pialsurf, dirnameSubj)
...
[mapMesh2Vox, fwmodel.pialsurf.mesh]  = projVoltoMesh_scalp(dirnameSubj, headvol.img, pialsurf, headvol.tiss_prop);
if isempty(mapMesh2Vox)
    return;
end

nNodes = size(fwmodel.pialsurf.mesh.vertices,1);
...
if fluenceProf.index==1
    ...
    fluenceProf.mesh = fwmodel.pialsurf.mesh;
end

filenm = sprintf('%sfluenceProf_scalp%d.mat', dirnameOut, fluenceProf.index);
