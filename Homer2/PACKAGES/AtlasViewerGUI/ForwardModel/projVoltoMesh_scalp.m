...
if isempty(fwmodel.projVoltoMesh_scalp)
    ...
    [nodeX, nNode, fwmodel.mesh_scalp] = reduceMesh(fwmodel, dirnameOut);
...
if size(elem,1)>40000
    idx = menu( sprintf('The head surface has %d faces. It is recommended that this be less than 40,000.\nShall I reduce this?',size(elem,1)),'Yes','No');
    if idx==1
        ...
        if idx==1
            ...
            elem = fvn.faces;
            nNode = size(nodeX,1);
            write_surf([dirnameOut, 'headsurf_sensitivity.mesh'], fvn.vertices, fvn.faces);
        else
