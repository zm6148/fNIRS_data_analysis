...
filename_path = 'C:\\Users\\mz86\\Desktop\\fNIRS ML\\data\\blvg_trace_exp2_all\\answer_combined\\all_answer_sheet.mat'
...
filename_path = 'C:\\Users\\mz86\\Desktop\\fNIRS ML\\data\\blvg_trace_exp2_all\\answer_combined\\CA_answer_sheet.mat'
#filename_path = 'C:\\Users\\mz86\\Desktop\\fNIRS ML\\data\\blvg_trace exp2\\answer\\answer_sheet.mat'
mat_answer = scipy.io.loadmat(filename_path)
answer_sheet_CA = mat_answer['CA_answer_sheet']

# DB combined answer sheet
filename_path = 'C:\\Users\\mz86\\Desktop\\fNIRS ML\\data\\blvg_trace_exp2_all\\answer_combined\\DB_answer_sheet.mat'
#filename_path = 'C:\\Users\\mz86\\Desktop\\fNIRS ML\\data\\blvg_trace exp2\\answer\\answer_sheet.mat'
mat_answer = scipy.io.loadmat(filename_path)
...
starting_index = 0
for ii in range(0,4):
    starting_index = ii * 28
    all_region_matrix[starting_index:starting_index+28, ii] = 1
...
for ii in range(0,2):
    starting_index = ii * 28
    CA_region_matrix[starting_index:starting_index+28, ii] = 1
    
# combine DB (cIFS)
DB_region_matrix = np.zeros((answer_sheet_DB.shape[0], 4))
starting_index = 0
for ii in range(2,4):
    starting_index = (ii-2) * 28
    DB_region_matrix[starting_index:starting_index+28, ii] = 1
...
for hh in range(0,1):
    ...
    for filename in os.listdir(dirname):
        ...
        if site_name == 'all_matrix':
            ...
            all_answer_sheet_index = all_index = np.array(range(0,answer_sheet.shape[0]))
            ITD_index = np.array(range(0,14) + range(28,42) + range(56,70) + range(84,98))
            noITD_index = np.delete(all_answer_sheet_index, ITD_index)
            
        elif site_name == 'CA_matrix':
            ...
            all_answer_sheet_index = all_index = np.array(range(0,answer_sheet.shape[0]))
            ITD_index = np.array(range(0,14) + range(28,42))
            noITD_index = np.delete(all_answer_sheet_index, ITD_index)
            
        elif site_name == 'DB_matrix':
        ...
        for n in selected_nn_index_loss:
            ...
            wave_coef_matrix_subject = np.concatenate((wave_coef_matrix[:,selected_feature_index], region_matrix), axis=1)
        ...
        per_correct_ITD = np.sum(perdiction[ITD_index].astype(float))/(row_num/2)
        per_correct_noITD = np.sum(perdiction[noITD_index].astype(float))/(row_num/2)
...
print ('nn_ITD:')
###############################################################################################
print ('all: ' + str(np.mean(all_accuracy_ITD[:,0])) + ' std: ' + str(np.std(all_accuracy_ITD[:,0])))
print ('CA: ' + str(np.mean(all_accuracy_ITD[:,1])) + ' std: ' + str(np.std(all_accuracy_ITD[:,1])))
print ('DB: ' + str(np.mean(all_accuracy_ITD[:,2])) + ' std: ' + str(np.std(all_accuracy_ITD[:,2])))
###############################################################################################
print ('nn_noITD:')
###############################################################################################
print ('all: ' + str(np.mean(all_accuracy_noITD[:,0])) + ' std: ' + str(np.std(all_accuracy_noITD[:,0])))
print ('CA: ' + str(np.mean(all_accuracy_noITD[:,1])) + ' std: ' + str(np.std(all_accuracy_noITD[:,1])))
print ('DB: ' + str(np.mean(all_accuracy_noITD[:,2])) + ' std: ' + str(np.std(all_accuracy_noITD[:,2])))

## save as npy files
# save the accuracy for later
np.save('C:\\Users\\mz86\\Desktop\\fNIRS ML\\test_on_exp2_results_v3\\all_accuracy_ITD.npy', all_accuracy_ITD)
# save the best lose for later
np.save('C:\\Users\\mz86\\Desktop\\fNIRS ML\\test_on_exp2_results_v3\\all_accuracy_noITD.npy', all_accuracy_noITD)
