...
for jj in range(0,100):
    ...
    for filename in os.listdir(dirname):
        ...
        for n in range(0,repeat_num):
            ...
            wave_coef_matrix_training_feature_selected_subject_columns = np.concatenate((wave_coef_matrix_training_feature_selected, subject_matrix_training), axis=1)
            ...
            for i in range(0, 10, 1):
                ...
                clf_nn = MLPClassifier(solver='lbfgs', activation='logistic', hidden_layer_sizes=(25,10,5),random_state=r,max_iter=1000)
                clf_nn.fit(X,y)
            ...
            clf_nn = MLPClassifier(solver='lbfgs', activation='logistic', hidden_layer_sizes=(25,10,5),random_state=bestseed,max_iter=1000)
            clf_nn.fit(X,y)
            
            # before making perdictions
            # add the subject column to testing matrix as well
            wave_coef_matrix_testig_subject = np.concatenate((wave_coef_matrix_testig[:,selected_feature_index], subject_matrix_testig), axis=1)
