...
for jj in range(0,100):
    ...
    for filename in os.listdir(dirname):
        ...
        # do feature selection on HbO and HbR separately
        # each chose 10? 25? test on it
        ...
        for n in range(0,repeat_num):
            ...
            all_subject_index = np.array(range(0, 14))
            
            # new training set and testing set
            # training set is from random 10 out of the total 14 subject
            # while the remaining 4 subject forms the testing set
            training_subject_index = random.sample(all_subject_index, 10)
            testing_subject_index = np.delete(all_subject_index, training_subject_index)
            
            # generate training index and testing index
            training_index = []
            for index_item in training_subject_index:
                training_index = training_index + range(index_item*10, index_item*10+10) + range(index_item*10 + 140, index_item*10 + 10 + 140)
            
            testing_index = []
            for index_item in testing_subject_index:
                testing_index = testing_index + range(index_item*10, index_item*10+10) + range(index_item*10 + 140, index_item*10 + 10 + 140)
