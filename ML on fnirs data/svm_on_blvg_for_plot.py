...
dirname = 'C:\\Users\\mz86\Desktop\\fNIRS ML\\data\\blvg_trace exp1\\6'
filename_list=[]
four_sit_result = []
four_sit_result_DT = []
four_sit_result_nn = []
for filename in os.listdir(dirname):
    ...
    for ii in range(0,repeat_num):
        ...
        for i in range(0, 10, 1):
            ...
            clf = MLPClassifier(solver='lbfgs', activation='logistic', hidden_layer_sizes=(20,10,10),random_state=r,max_iter=1000)
            clf.fit(X,y)
            
        if(clf.loss_ < bestloss):
            bestloss = clf.loss_
            bestseed = r
        print(clf.loss_, bestloss)
        
        clf = MLPClassifier(solver='lbfgs', activation='logistic', hidden_layer_sizes=(20,10,10),random_state=bestseed,max_iter=1000)
        clf.fit(X,y)
        
        prediction = clf.predict(wave_coef_matrix_testig[:,selected_feature_index])
