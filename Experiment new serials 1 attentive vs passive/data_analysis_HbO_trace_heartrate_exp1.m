...
for index=1:length(files)
    ...
    % add heart rate thing here %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % dod_raw is the raw signal
    % method_heart_rate to analysis 
    % output matrix of block average of heart rate response
    ...
    % mean heart rate
    [mean_heartrate_at_C, data_original, pks, locs, all_channel_HRV] = method_heart_rate(dod_raw, s_task, t, window_h, window_size);
    ...
    % mean heart rate
    mean_heartrate_at_A = method_heart_rate(dod_raw, s_task, t, window_h, window_size);
    ...
    % mean heart rate
    mean_heartrate_at_D = method_heart_rate(dod_raw, s_task, t, window_h, window_size);
    ...
    % mean heart rate
    mean_heartrate_at_B = method_heart_rate(dod_raw, s_task, t, window_h, window_size);
...
%         mean(breath_blockAvg_subject_AB,3));
    ...
    heartrate_blockAvg_subject = cat (4, mean_heartrate_at_C, ...
        mean_heartrate_at_A, ...
        mean_heartrate_at_D, ...
        mean_heartrate_at_B);
...
%         dcAvg_subject_ITD_noITD_diff_AB);
    ...
    heartrate_blockAvg_all = cat(5, heartrate_blockAvg_all, heartrate_blockAvg_subject);
...
b = figure(2);
t = (window_h(1):1/50:window_h(2))';
region = ['C', 'A', 'D', 'B'];

for ii = 1 : size(heartrate_blockAvg_all,4)
    
    % each region has different channels selected
    subject_selected = [1:14]; %both_selected{ii};
  
    % HbO 1
    task_site_matrix_HbO = reshape(heartrate_blockAvg_all(:,1, 1, ii,subject_selected), [size(heartrate_blockAvg_all,1),length(subject_selected)]);
